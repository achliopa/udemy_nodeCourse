...
io.on('connection', (socket) => {
	...
	socket.on('join', (params, callback) => {
			...
			// io.emit() -> io.to('A Room').emit()
			// socket.broadcast.emit() -> socket.broadcast.to('A Room').emit()
			// socket.emit() -> same
	...
	socket.on('disconnect', () => {
		...
		io.to(user.room).emit('newMessage', generateMessage('Admin', `@${user.name} left #${user.room}`));
	});
})
